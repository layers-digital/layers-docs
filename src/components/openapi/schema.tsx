...
export class DocOpenapiSchema {
  ...
  buildIsRequiredLabel(node: AcessorNode) {
    ...
    if (isRequired) {
      return <span class="Api-label Api-label-required">obrigatório</span>
    }
  }
  ...
  hasProperties(node: AcessorNode) {
    ...
    //   })
    // }
      return this.hasProperties({name: '[]', schema: normalizeObject(this.spec, node.schema.items)}) || (node.schema.items as SchemaObject).oneOf
    }
